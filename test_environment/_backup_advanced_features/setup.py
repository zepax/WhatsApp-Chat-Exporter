...
def install_dependencies(install_type="full"):
    ...
    # Base requirements
    base_requirements = ["beautifulsoup4>=4.12.0", "lxml>=4.9.0", "pandas>=1.5.0"]
...
def setup_textblob():
    ...
    try:
        ...
        # Download corpora
        subprocess.run(
            [sys.executable, "-m", "textblob.download_corpora"], capture_output=True
        )
...
def create_default_config():
    ...
    default_config = {
        ...
        "scheduling": {"enabled": False, "frequency": "weekly", "time": "02:00"},
    }
...
def run_system_checks():
    ...
    try:
        ...
        if memory.total >= 2 * 1024 * 1024 * 1024:  # 2GB
            ...
            print(
                f"⚠️  Low memory: {memory.total // (1024**3):.1f} GB (2GB+ recommended)"
            )
    except:
...
def main():
    ...
    parser.add_argument(
        "--no-optional", action="store_true", help="Skip optional feature installation"
    )
    ...
    print(
        "3. For web dashboard: python content_analyzer.py /path/to/chats --generate-dashboard"
    )
