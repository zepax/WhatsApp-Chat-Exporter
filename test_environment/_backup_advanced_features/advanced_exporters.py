...
def generate_advanced_excel_report(
    analyzer_instance, output_dir: str = "analysis_results"
):
    """Generate comprehensive Excel report with multiple sheets and charts."""
    if not EXCEL_ADVANCED:
        logging.warning(
            "Advanced Excel features not available. Install openpyxl for full functionality."
        )
        return None
    ...
    try:
        ...
        summary_data = [
            ...
            (
                ...
                f"{summary.get('total_files_analyzed', 0):,}",
            ),
            ("💬 Total de mensajes:", f"{summary.get('total_messages', 0):,}"),
            ("📝 Total de palabras:", f"{summary.get('total_words', 0):,}"),
            (
                "🎯 Archivos con coincidencias:",
                f"{summary.get('files_with_keyword_matches', 0):,}",
            ),
            (
                "⚡ Promedio mensajes por archivo:",
                f"{summary.get('average_messages_per_file', 0):.1f}",
            ),
            (
                "📊 Promedio palabras por archivo:",
                f"{summary.get('average_words_per_file', 0):.1f}",
            ),
        ]
        ...
        for keyword, count in list(top_keywords.items())[:10]:
            percentage = (
                (count / total_keyword_count * 100) if total_keyword_count > 0 else 0
            )
            ...
            for col, keyword in enumerate(all_keywords, 2):
                ...
                if count > 0:
                    if count >= 10:
                        ...
                        cell.fill = PatternFill(
                            start_color="6BCF7F", end_color="6BCF7F", fill_type="solid"
                        )
        ...
        if top_keywords:
            ...
            data_ref = Reference(
                ws_charts, min_col=2, min_row=1, max_row=len(chart_keywords) + 1
            )
            cats_ref = Reference(
                ws_charts, min_col=1, min_row=2, max_row=len(chart_keywords) + 1
            )
...
def generate_pdf_report(analyzer_instance, output_dir: str = "analysis_results"):
    ...
    if not PDF_AVAILABLE:
        logging.warning(
            "PDF generation not available. Install reportlab for PDF functionality."
        )
        return None
    ...
    try:
        ...
        # Title
        story.append(
            Paragraph("📊 REPORTE DE ANÁLISIS DE CONVERSACIONES WHATSAPP", title_style)
        )
        ...
        if top_keywords:
            ...
            keyword_table = Table(
                keyword_data, colWidths=[2 * inch, 1 * inch, 1 * inch]
            )
        ...
        if conv_breakdown:
            ...
            conv_table = Table(
                conv_data, colWidths=[3 * inch, 1 * inch, 1 * inch, 1 * inch]
            )
            conv_table.setStyle(
                TableStyle(
                    [
...
def generate_csv_export(analyzer_instance, output_dir: str = "analysis_results"):
    ...
    if not PANDAS_AVAILABLE:
        logging.warning(
            "CSV export requires pandas. Install pandas for full functionality."
        )
        return None
