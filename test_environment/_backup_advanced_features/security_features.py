...
class SecurityManager:
    ...
    def validate_data_retention(self, file_path: str, max_age_days: int = 90) -> bool:
        ...
        try:
            file_age = (
                datetime.now() - datetime.fromtimestamp(Path(file_path).stat().st_mtime)
            ).days
    ...
    def generate_privacy_report(self, analyzer_instance, output_dir: str) -> str:
        ...
        try:
            ...
            anonymized_files = sum(
                1 for r in analyzer_instance.results if getattr(r, "anonymized", False)
            )
            encrypted_outputs = sum(
                1 for r in analyzer_instance.results if getattr(r, "encrypted", False)
            )
            ...
            report_data = {
                ...
                "recommendations": [
                    (
                        "Enable anonymization for production use"
                        if not analyzer_instance.config.anonymize_data
                        else "Anonymization properly enabled"
                    ),
                    (
                        "Enable output encryption for sensitive data"
                        if not analyzer_instance.config.encrypt_output
                        else "Output encryption enabled"
                    ),
...
class DataProtectionManager:
    ...
    def record_consent(self, data_subject: str, purpose: str, consent_given: bool):
        ...
        consent_id = hashlib.sha256(f"{data_subject}_{purpose}".encode()).hexdigest()[
            :16
        ]
    ...
    def apply_data_minimization(self, analyzer_instance) -> Dict[str, Any]:
        ...
        removed_keywords = [
            k for k in analyzer_instance.config.keywords if k not in essential_keywords
        ]
        ...
        minimization_report.update(
            {
                ...
                "data_reduction_percentage": (
                    len(removed_keywords) / len(analyzer_instance.config.keywords)
                )
                * 100,
            }
        )
        ...
        self.logger.info(
            f"Data minimization applied: {len(removed_keywords)} keywords removed"
        )
        return minimization_report

    def generate_data_processing_record(
        self, analyzer_instance, output_dir: str
    ) -> str:
            ...
            processing_record = {
                ...
                "processing_statistics": {
                    ...
                    "total_messages_analyzed": sum(
                        r.total_messages for r in analyzer_instance.results
                    ),
...
def apply_security_features(
    analyzer_instance, security_config: Dict[str, Any]
) -> SecurityManager:
        ...
        if security_config.get("anonymize_data", False):
            for result in analyzer_instance.results:
                if hasattr(result, "participants"):
                    result.participants = security_manager.anonymize_participants(
                        result.participants
                    )
