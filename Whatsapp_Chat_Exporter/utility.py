...
def get_cond_for_empty_secure(enable: bool, jid_field: str, broadcast_field: str) -> str:
    """Generate a SQL condition for filtering empty chats with validation."""

    if not enable:
        return ""

    pattern = r"^[a-zA-Z_][a-zA-Z0-9_]*(\.[a-zA-Z_][a-zA-Z0-9_]*)?$"
    if not re.match(pattern, jid_field):
        raise ValueError(f"Invalid JID field name: {jid_field}")
    if not re.match(pattern, broadcast_field):
        raise ValueError(f"Invalid broadcast field name: {broadcast_field}")

    return (
        f"AND (chat.hidden=0 OR {jid_field}='status@broadcast'"
        f" OR {broadcast_field}>0)"
    )


def get_cond_for_empty(enable: bool, jid_field: str, broadcast_field: str) -> str:
    """Backward compatible wrapper for ``get_cond_for_empty_secure``."""

    return get_cond_for_empty_secure(enable, jid_field, broadcast_field)


def get_chat_condition_secure(
    ...
    """Generate a parameterized SQL condition for chat filtering.
    ...
    if jid is not None and not re.match(pattern, jid):
...
def get_chat_condition(
    ...
    """Backward compatible wrapper returning only the condition string."""

    cond, _ = get_chat_condition_secure(filter, include, columns, jid, platform)
    return cond
