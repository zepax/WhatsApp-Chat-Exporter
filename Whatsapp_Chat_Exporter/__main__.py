...
def validate_args(parser: ArgumentParser, args) -> None:
    ...
    if not args.android and not args.ios and not args.exported and not args.import_json:
        if args.backup:
            ...
            device, detected_path = detect(args.backup)
            if device is Device.ANDROID:
                args.android = True
                args.backup = detected_path
            elif device is Device.IOS:
                args.ios = True
                args.backup = detected_path
            else:
                parser.error("Could not detect the device type from backup.")
        else:
...
def main():
    ...
    # Automatic device detection when backup is specified
    if args.backup and not any([args.android, args.ios]):
        from Whatsapp_Chat_Exporter.backup_detector import detect
